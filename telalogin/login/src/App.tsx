...
export function App() {
  ...
  const handleLoginSubmit = (data: NewLoginFormData) => {
    ...
    const user = dados.users.find((user: any) => user.email === email && user.password === password);
    ...
    if (user) {
      ...
      setErrorMessage(errorMessage); // Armazenar mensagem de erro no estado
    }
        ...
        <div>
          <main className="flex flex-col mt-2 gap-10 w-full max-w-[384px]">
            <header className="flex flex-col gap-4 w-full max-w-[350px]">
              <h1 className="font-sans text-4xl font-bold text-gray-800">Tela de Login</h1>
              <p className="font-sans font-normal text-base text-gray-600">Login</p>
            </header>
